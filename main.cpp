...
//
void ComputePyramid(const int levels_num, cv::Mat &img, vector<float> &vec_scale_per_factor, std::vector<cv::Mat> &vec_img_pyramid){
...
int main() {
    ...
    for (int level = 0; level < levels_num; ++level) {
        ...
        cout << "第" << level + 1 << "层图像被切割成 " << rows << " 行 " << cols << " 列. 格子有 " << width_cell << " 列 " << height_cell << " 行 ";
