...
int main() {
  cv::Mat img = cv::imread("/home/sph/CLionProjects/distribute_oct_tree/luna.jpg", CV_LOAD_IMAGE_COLOR);
  if (img.empty()) {
  ...
  std::vector<cv::Mat> vec_img_pyramid(levels_num);
  ...
  for (int level = 0; level < levels_num; ++level) {
    ...
    cout << "第" << level + 1 << "层图像被切割成 " << rows << " 行，" << cols << " 列" << endl;
    cout << "格子列数： " << width_cell << "， 格子行数：" << height_cell << endl;
    ...
    cout << "这层图像共有 " << vec_to_per_distribute_keys.size() << " 个特征点" << endl;
    ...
    cout << "初始化时有 " << init_node_num << " 个节点" << endl;
